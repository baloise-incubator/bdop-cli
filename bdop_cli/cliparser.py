...
from bdop_cli.io_api.yaml_util import yaml_load, YAMLException
...
def __add_git_credentials_args(deploy_p: ArgumentParser) -> None:
    deploy_p.add_argument(
        "--username",
        help="Git username (alternative: GIT_USERNAME env variable)",
        required="GIT_USERNAME" not in os.environ,
        default=os.environ.get("GIT_USERNAME"),
    )
    deploy_p.add_argument(
        "--password",
        help="Git password or token (alternative: GIT_PASSWORD env variable)",
        required="GIT_PASSWORD" not in os.environ,
        default=os.environ.get("GIT_PASSWORD"),
    )


def __add_git_commit_user_args(deploy_p: ArgumentParser) -> None:
    deploy_p.add_argument("--git-user", help="Git Username", default="bdop")
    deploy_p.add_argument("--git-email", help="Git User Email", default="bdop@baloise.dev")
